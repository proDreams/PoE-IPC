...
class Parse:
    urls = {
        "en": "https://www.pathofexile.com/trade/about",
    ...
    def parse(self, actual_league, poesessid):
        ...
        for lang in self.langs:
            ...
            for category in second_point:
                if (
                    "Item Tags" in category.text
                    and "Maps" not in category.text
                    and "Cards" not in category.text
                ) or (
                    "Тэги предмета - " in category.text
                    and "Карты" not in category.text
                    and "Гадальные карты" not in category.text
                ):
                    category_name = category.text.replace("Item Tags - ", "").replace(
                        "Тэги предмета - ", ""
                    )
                    ...
                    for item in items:
                        item_name = (
                            item.find_element("class name", "filter-title")
                            .get_attribute("textContent")
                            .strip()
                        )
                        item_alt_name = item.find_element(
                        ...
                        if img:
                            ...
                            directory = f"assets/img/{category_name}"
                            if not os.path.exists(directory):
                                os.makedirs(directory)
                            with open(
                                f"{directory}/{item_alt_name}.png", "wb"
                            ) as image:
                            ...
                            result_dic[lang][category_name].update(
                                {
                                    item_name: {
                                        ...
                                        "img": f"assets/img/{category_name}/{item_alt_name}.png",
                                    }
                                }
                            )
                        else:
...
def get_items(lang, category):
    with open("items.json", "r") as jsonFile:
        items = json.load(jsonFile)
        return [
            [key, items[lang][category][key]["img"]]
            for key in items[lang][category].keys()
        ]
...
def check_price(want, league, quant=1, have="chaos"):
    ...
    try:
        total_price = round(
            (stat.mean(price_catalog[2:]) + stat.median(price_catalog[2:])) / 2, 4
        )
