...
class Settings(UserControl):
    def __init__(self):
        ...
        self.lang_label = Text(
            text_constants.settings_language_text[text_constants.current_lang]["label"],
            size=18,
            weight="bold",
        )

        self.lang_change = Text(
            text_constants.settings_language_text[text_constants.current_lang]["change"]
        )
        self.lang_dd = Dropdown(
            ...
            options=[
                dropdown.Option(
                    key="ru",
                    text=text_constants.settings_language_text[
            ...
            label=text_constants.settings_language_text[text_constants.current_lang][
                text_constants.current_lang
            ],
            border_radius=5,
        ...
        self.mode_label = Text(
            text_constants.settings_mode_text[text_constants.current_lang]["label"],
            size=18,
            weight="bold",
        )

        self.mode_change = Text(
            text_constants.settings_mode_text[text_constants.current_lang]["change"]
        )
        self.mode_dd = Dropdown(
            ...
            options=[
                dropdown.Option(
                    key="bulk",
                    text=text_constants.settings_mode_text[text_constants.current_lang][
                        "bulk"
                    ],
                ),
                dropdown.Option(
            ...
            label=text_constants.settings_mode_text[text_constants.current_lang][
        ...
        self.league_label = Text(
            text_constants.settings_league_text[text_constants.current_lang]["label"],
            size=18,
            weight="bold",
        )

        self.actual_label = Text(
            text_constants.settings_league_text[text_constants.current_lang]["actual"],
            width=200,
        )
        self.actual_content = Text(self.leagues_list[4])
        self.actual_row = Row(
            [self.actual_label, self.actual_content], alignment=MainAxisAlignment.CENTER
        )

        self.selected_content = Text(text_constants.selected_league)

        self.league_change = Text(
            text_constants.settings_league_text[text_constants.current_lang]["change"]
        )
    ...
    def change_lang(self, e):
        if text_constants.current_lang != self.lang_dd.value:
            ...
            dlg = AlertDialog(
                title=Text(
                    text_constants.settings_dialog_text[self.lang_dd.value][
                        "restart_app"
                    ]
                )
            )
...
class SettingsVersion(UserControl):
    def __init__(self):
        super().__init__()
        self.label = Text(
            text_constants.settings_version_text[text_constants.current_lang]["label"],
            size=18,
            weight="bold",
        )

        self.server_base_label = Text(
            text_constants.settings_version_text[text_constants.current_lang]["server"],
            width=250,
        )
        self.server_base_content = Text(text_constants.server_base)
        self.server_base_row = Row(
            [self.server_base_label, self.server_base_content],
            alignment=MainAxisAlignment.CENTER,
        )

        self.local_base_label = Text(
            text_constants.settings_version_text[text_constants.current_lang]["local"],
            width=250,
        )
        self.local_base_content = Text(text_constants.local_base)
        self.local_base_row = Row(
            [self.local_base_label, self.local_base_content],
            alignment=MainAxisAlignment.CENTER,
        )

        self.update_required_label = Text(
            text_constants.settings_version_text[text_constants.current_lang][
                "need_update"
            ],
            width=220,
        )
        ...
        self.update_required_row = Row(
            [self.update_required_label, self.update_required_content],
            alignment=MainAxisAlignment.CENTER,
        )
        ...
        self.poesessid_input = TextField(
            label=text_constants.settings_version_text[text_constants.current_lang][
                "poesessid_input"
            ],
            height=60,
        )
        ...
        self.input_row = Row(
            [self.poesessid_input, self.poesessid_submit], visible=False
        )
    ...
    def server_update(self, e):
        if text_constants.server_base == text_constants.local_base:
            dlg = AlertDialog(
                title=Text(
                    text_constants.settings_dialog_text[text_constants.current_lang][
                        "update_not_required"
                    ]
                )
            )
    ...
    def local_update(self, e):
        ...
        self.update()
        utils.Parse().parse(
            actual_league=text_constants.list_leagues[4],
